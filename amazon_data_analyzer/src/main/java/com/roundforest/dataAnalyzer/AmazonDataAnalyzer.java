...
public class AmazonDataAnalyzer {
	...
	public AmazonDataAnalyzer() {
		...
		// create spark Session using as many worker threads as logical cores on the machine
		sparkSession = SparkSession.builder().master("local[*]").appName("RoundForest Data Analyzer").getOrCreate();
	}
	...
	public boolean readFile(String csvFile) {
		...
		try {
			ds = sparkSession.read().option("header", "true").csv(csvFile).dropDuplicates(); //removing duplicate records from the dataset.
			bSuccessful = true; 
		} catch (Exception e) {
